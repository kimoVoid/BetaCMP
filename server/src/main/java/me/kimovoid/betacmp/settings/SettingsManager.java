...
public class SettingsManager {
    ...
    public static void applyConf() {
        ...
        for (Map.Entry<String, String> entry : conf.entrySet()) {
            if (!rules.get(entry.getKey().toLowerCase()).setValue(entry.getValue())) {
                ...
                LOGGER.info("Loaded rule " + entry.getKey() + " as " + entry.getValue() + " from betacmp.conf");
            }
        }
    ...
    private static Map<String, String> readConf() {
        try {
            File rules_file = BetaCMP.SERVER.getFile("config/betacmp.conf");
    ...
    private static void writeConf(Map<String, String> values) {
        ...
        try {
            File config = BetaCMP.SERVER.getFile("config");
            if(!config.exists()) {
                config.mkdir();
            }
            File rules_file = BetaCMP.SERVER.getFile("config/betacmp.conf");
            ...
            e.printStackTrace();
            LOGGER.error("Failed to write betacmp.conf");
        }
    }
